...
const IconChecked = () => {
  return (
    <img className='w-[15px] mx-auto' src={iconchecked} alt='icon checked'/>
  )
}

const IconFalse = () => {
  return (
    <img className='w-[15px] mx-auto' src={iconfalse} alt='icon false'/>
  )
}
            ...
            <tbody>
              {
                detailData.map((item, index) =>
                  <tr key={index+item.name}>
                    <td className='p-8 text-[.9rem] ' style={
                      index === 2 || index === 3 || index === 4 ? {fontWeight: '600'} :{}
                    }>
                      {item.name}
                    </td>
                    <td className='text-center bg-[#F6F1D0]'>{
                      item.eksklusif ? 
                      <IconChecked/> :
                      <IconFalse/>
                    }</td>
                    ...
                    <td className='text-center bg-[#CDEDFE]'>{
                      item.premium ? 
                      <IconChecked/> :
                      <IconFalse/>
                    }</td>
                    ...
                    <td className='text-center font-semibold bg-[#CDEDFE]'>{
                      item.basic === '*Optional' ? 
                      item.basic :
                      item.basic ? 
                      <IconChecked/> :
              ...
              <thead>
                <tr className='text-white'>
                  ...
                  <td 
                    ...
                    style={
                      btn === 'Eksklusif' ? {backgroundColor: '#F4B14A'} :
                      btn === 'Platinum' || btn === 'Standard' ? {backgroundColor: '#28b7fb'} :
                      btn === 'Premium' || btn === 'Basic' ? {backgroundColor: '#2f98d6'} : {}
                    }>{
                    btn
                  }</td>
                </tr>
              ...
              <tbody>
                {
                  ...
                  <tr key={index+'mobiletable'}>
                    ...
                    <td 
                      ...
                      style={
                        btn === 'Eksklusif' ? {backgroundColor: '#F6F1D0'} :
                        btn === 'Premium' || btn === 'Basic' ? {backgroundColor: '#CDEDFE'} : {}
                      }
                    >{
                      btn === 'Eksklusif' ?
                        item.eksklusif ? 
                          <IconChecked/> : <IconFalse/>
                      : 
                      btn === 'Platinum' ?
                        item.platinum === '*Optional' ? 
                        item.platinum :
